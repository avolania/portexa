...
export default function MyTicketsPage() {
  ...
  // Sadece bu kullanıcının talepleri
  const myIncidents = incidents.filter(
    (i) => i.callerId === user.id || i.reportedById === user.id
  );
  const mySRs = serviceRequests.filter(
    (sr) => sr.requestedById === user.id
  );
  ...
  // Birleşik liste — tarihe göre sırala
    ...
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      <div>
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
          <Link href="/itsm/portal" className="hover:text-indigo-600">Destek Portalı</Link>
        ...
        <div className="flex items-center justify-between">
          ...
          <Link href="/itsm/portal" className="btn-primary text-sm">+ Yeni Talep</Link>
        </div>
      </div>
      ...
      <div className="card p-0 overflow-hidden">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3 text-gray-400">
            ...
            <p className="text-sm">Henüz {tab === "open" ? "açık" : tab === "closed" ? "kapalı" : "hiç"} talebiniz yok.</p>
            <Link href="/itsm/portal" className="text-sm text-indigo-600 hover:underline">
